...
class LoginIntegrationTest extends IntegrationSupporter {
	...
	void refresh_token을_이용해_access_token을_재발급한다() {
		final ExtractableResponse<Response> response = post("/api/access-token/issue", generateRefreshHeader(member.getRefreshToken()), KAKAO_MEMBER_TYPE);
	...
	@Test
	void 로그아웃을_진행한다() {
		final ExtractableResponse<Response> response = post("/api/logout", generateAccessHeader(access_token), KAKAO_MEMBER_TYPE);
